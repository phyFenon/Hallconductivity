...
use omp_lib
Implicit None
Integer, Parameter :: width=100,n=2*width,nEpsilon=100,stepsx=200,stepsz=200! width->the landau levels   
Double Precision, Parameter :: pi = atan(1.d0)*4
integer,Dimension(10)::nB
complex *16,allocatable, dimension(:,:)::hall
complex *16::hall_singlek
double precision::kx,kz,B,t1,t2,mu,eta
integer::i,j,k,fileindex  
    

Open (10, File='m=1_t0=0.0_mu=0.0_OBC_Bvary.txt')

nB=(/10000,110,100,95,90,80,70,60,50,45/)
...
read*,fileindex
B=2*pi/nB(fileindex) !width is the total layer of Y direction
print*,B
allocate(hall(stepsx,stepsz))

!$omp parallel do private(B,j,kz,i,kx,mu,hall_singlek)
DO j=1,stepsz
...
hall(i,j)=cmplx(0.0,1.0)*hall_singlek
enddo
enddo
...
write(10,100)B,real(sum(hall(:,:)))/(stepsx*stepsz),aimag(sum(hall(:,:)))/(stepsx*stepsz)
...
Double Precision, Parameter :: pi = atan(1.d0)*4
Complex * 16, Dimension (n, n) :: H,Vx,Vy
...
!--------------Vx-----------------------!
do k=1,width
Vx(2*k-1:2*k,2*k-1:2*k)= cos(kx+k*B)*sigmax+sin(kx+k*B)*sigmaz
end do 
!------------the end--------------------!
!--------------Vy-----------------------!
do k=1,width-1
...
if(i.ne.j) then
phase_ij=phase_ij+matrix(n,col_i,Vx,col_j)*matrix(n,col_j,Vy,col_i)&
